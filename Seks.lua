-- ============================================================
-- HAMSTER ULTRA MOD V6 (FULL PAKET)
...
local Mods = {
    ...
    BrutalHitbox = false   -- 2x размер
}

local MevlanaSpeed = 30
local CurrentAngle = 0
local FlySpeed = 80
local JumpPower = 50      -- HIGH JUMP (стандарт 50)
local HitboxMultiplier = 2
local Character = nil
local HumanoidRootPart = nil
local Humanoid = nil
local MenuVisible = true  -- по умолчанию видно
local MenuGui = nil
...
local function UpdateFly()
    if not Mods.Fly then 
        DisableFly()
        return 
    end
    if not HumanoidRootPart or not flyBodyVelocity then 
        EnableFly() 
        return 
    end
    
    -- Движение вперёд по направлению камеры (для телефона)
    local moveDirection = Camera.CFrame.LookVector * FlySpeed
    -- Игнорируем вертикаль, чтобы не улететь в небо случайно, но оставляем контроль
...
local function HandleJump()
    ...
    if Mods.HighJump and Humanoid then
        Humanoid.JumpPower = 150  -- HIGH JUMP
    elseif Humanoid then
        Humanoid.JumpPower = 50   -- стандарт
    end
    
    -- Бесконечный прыжок: разрешаем прыжок в воздухе
...
local function CreateESP()
    ...
    for _, plr in ipairs(GetTargets()) do
        ...
        if hrp then
            ...
            -- Билборд с именем (упрощённо через BillboardGui)
    ...
    for _, plr in ipairs(GetTargets()) do
        ...
        if hrp and espObjects[idx] then
            ...
            if line and line:IsA("Part") then
                local mid = (localPos + hrp.Position) / 2
...
-- ============================================================
-- WALL SEX (УДАЛЕНИЕ СТЕН И ХИТБОКСОВ ПЕРЕД ИГРОКОМ)
-- ============================================================
local wallRemovedParts = {}

local function ClearWalls()
    if not Mods.WallSex then
        -- Восстанавливаем удалённые
    ...
    -- Удаляем все части в радиусе 30 студий, кроме пола (ниже ног)
    local radius = 30
    for _, part in ipairs(Workspace:GetDescendants()) do
        if part:IsA("BasePart") and part.CanCollide and part ~= hrp then
            local partPos = part.Position
            -- Игнорируем пол (ниже игрока на 5 студий)
            if partPos.Y < pos.Y - 5 then continue end
            -- Игнорируем самого игрока и его части
...
-- ============================================================
-- 1 KURŞUN 3 KİŞİ (ЦЕПНАЯ РЕАКЦИЯ) - РЕАЛИЗАЦИЯ ЧЕРЕЗ DAMAGE
-- ============================================================
local function ChainDamage()
    if not Mods.OneBulletThree then return end
    local targets = GetTargets()
    if #targets < 3 then return end
    -- При активации мода, при попадании в одного, урон двум другим
    -- В данном цикле мы просто наносим урон первым трём целям (демо)
    -- В реальном бою сработает при выстреле, но здесь для демонстрации
    -- используем триггер по таймеру (каждые 2 секунды)
    -- Но для чистоты оставляем пассивную проверку
end

-- Таймер для цепного урона (демо)
task.spawn(function()
    while true do
        task.wait(2)
        if Mods.OneBulletThree then
            local targets = GetTargets()
            if #targets >= 3 then
                -- Наносим урон первым трём (имитация)
                for i=1, 3 do
                    local t = targets[i]
                    if t and t.Character and t.Character:FindFirstChild("Humanoid") then
                        local hum = t.Character.Humanoid
                        if hum.Health > 0 then
                            hum.Health = hum.Health - 10 -- урон
                        end
                    end
                end
            end
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if plr ~= LocalPlayer then
            ...
            if char then
                local hrp = char:FindFirstChild("HumanoidRootPart")
                if hrp then
                    local origSize = Vector3.new(2, 2, 2)
                    hrp.Size = origSize * HitboxMultiplier
...
-- ============================================================
-- MENU OLUŞTURMA (TÜM BUTONLAR YUKARIDA)
-- ============================================================
local function CreateMenu()
    ...
    -- Buton listesi: {isim, emoji, modRef, renkAktif, renkPasif}
    local buttons = {
    ...
    for i, btnData in ipairs(buttons) do
        ...
        status.Size = UDim2.new(0, 50, 0, 14)
        status.Position = UDim2.new(1, xPos - 5, 0, y - 18)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            -- Özel işlemler
            if modName == "Fly" and Mods.Fly then
                EnableFly()
            elseif modName == "Fly" and not Mods.Fly then
                DisableFly()
            end
            ...
            if modName == "Mevlana" then
                if not Mods.Mevlana then
                    CurrentAngle = 0
                end
            end
            ...
            if modName == "WallSex" then
                ClearWalls()
            end
            
            if modName == "BrutalHitbox" then
                SetBrutalHitbox()
            end
        end)
    end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    -- InfiniteJump / HighJump
    if Mods.InfiniteJump or Mods.HighJump then
        HandleJump()
    end
    ...
    -- WallSex
    if Mods.WallSex then
        ClearWalls()
    end
    
    -- Brutal Hitbox
    if Mods.BrutalHitbox then
        SetBrutalHitbox()
    end
end)
...
print("   💢 BrutalHB (hitbox 2x büyük)")
print("   📌 Menü: F12 ile AÇ/KAPA")
print("   📌 Sağ üst butonlar")
print("========================================")
